...
const PaymentInfo = () => {
    ...
    const handleDelete = async (id) => {
        ...
        const token = localStorage.getItem('token');
        await fetch(`${import.meta.env.VITE_API_URL}/api/payments/delete-card/${id}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${token}` },
        });
        fetchCards();
    ...
    const ToggleSwitch = ({ checked, onChange }) => (
        <button
            ...
            onClick={onChange}
            className={`relative w-12 h-7 rounded-full transition-colors duration-150 ${checked ? 'bg-[#ff3c33]' : 'bg-[#bbb]'
                }`}
            style={{ outline: 'none', border: 'none', padding: 0 }}
        >
            <span
                className={`absolute left-1 top-1 w-5 h-5 rounded-full bg-white transition-transform duration-150 ${checked ? 'translate-x-5' : ''
                    }`}
                style={{
